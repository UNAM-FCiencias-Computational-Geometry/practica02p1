...
module Test
  ...
  class TestRBTree
    ...
    def self.rb_min
      ...
      min = RBTree.rb_min(tree)
      
      raw_min = raw_numbers.sort!{|x,y| x<=>y}.first
    ...
    def self.rb_max
      ...
      max = RBTree.rb_max(tree)
      
      raw_max = raw_numbers.sort{|x,y| y<=>x}.first
    ...
    def self.rb_search
      ...
      search = RBTree.rb_search(tree,raw_search)
    ...
    def self.insert_delete_repeat_numbers
      ...
      raw_numbers.size.times.each do
        point = RBTree.rb_min(tree)
        point = RBTree.rb_delete(tree,point)
        order_points << point[:x]
      end
